...
A separate provider-enabled ChatGPT `check_domains` call returned GoDaddy evidence for public registered `.com`/`.fr` controls and available synthetic `.com`/`.fr` candidates. Registration and renewal prices, currency and provider check time were present for the available results; no provider error was reported. A provider-enabled `screen_names` call with `match="all"` returned one eligible synthetic candidate, and its real ChatGPT card displayed both extensions, GoDaddy, indicative registration/renewal prices and freshness. The ChatGPT model said it could not inspect the View, but the browser observation confirmed the rendered card. This completes the private provider-to-ChatGPT path for the tested cases only. No raw response, credential or private search was committed. The PAT's granted scopes were not independently verified. The local tunnel was stopped and the process credentials and matching clipboard contents were cleared after the test; the private Platform tunnel/App resources still exist.

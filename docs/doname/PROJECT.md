...
### MCP Apps presentation iteration
...
The resource URI is now `ui://doname/cards/v2.html` because its HTML/CSS changed incompatibly. The View reads theme, locale, timezone, platform and display mode from host context, re-renders on context changes, fits the host's container width, and declares both inline and fullscreen support. The host still decides where the View appears. The structured and text tool results are unchanged, and UI remains optional. A dedicated search box is deferred because the chat composer already provides name ideation and the V1 UI should focus on comparing checked results; a direct exact-domain check control can be assessed separately.

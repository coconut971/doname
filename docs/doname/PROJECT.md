...
### MCP Apps presentation iteration (v2, 2026-09-24)

2026-09-24 — The review branch `feat/refine-mcp-cards` replaces the first compact card layout with grouped candidate cards and separate extension tiles. Registration and renewal prices have distinct hierarchy; provider, availability state and relative check freshness stay visible, while exact evidence timestamps and source statuses remain expandable. French and English labels follow the MCP host locale, prices and dates use that locale's formatting, and the original provider currency is preserved. DoName does not infer location from IP or silently convert currencies; local-currency conversion would require a disclosed, dated FX source.

At that time, the resource URI changed to `ui://doname/cards/v2.html` because its HTML/CSS changed incompatibly. The View read theme, locale, timezone, platform and display mode from host context, re-rendered on context changes, fit the host's container width, and declared both inline and fullscreen support. The host still decides where the View appears. The structured and text tool results were unchanged, and UI remained optional. The original iteration deferred a dedicated search box because the chat composer already provides name ideation; a direct exact-domain check control was left for separate assessment.

GoDaddy is attributed by provider name only. The current [GoDaddy API Terms of Use](https://www.godaddy.com/en/legal/agreements/godaddy-api-terms-of-use) prohibit implying endorsement or affiliation without authorization, so no GoDaddy logo is included unless the required permission and brand guidance are obtained. The same terms restrict providing API access to third parties without prior written authorization. DoName makes no assumption about how those terms apply to its user-supplied-credential local plugin; confirm that distribution model with GoDaddy. Any operated shared-credential service requires written clearance before it is offered.

The local Windows release gate passed after the bundle smoke test was changed to discover the resource URI from the MCP tool metadata. It ran 38 Python tests under each of Python 3.10 and 3.13, manifest validation, the MCP Apps lifecycle/locale/theme smoke, extracted-bundle startup and tool discovery, package builds and artifact audit. No live provider request was made. The redesigned cards have not yet been visually re-tested in ChatGPT; the lifecycle and render behavior is currently verified with the host-bridge smoke only.

### MCP Apps presentation iteration (v3, 2026-09-25)

The V3 card removes the manual “Available only” control. `screen_names` still accepts the AI-selected availability criteria, and the View renders only eligible candidates; the model can explain omitted candidates in chat. The visual hierarchy is reduced to the candidate name, a compact available-extension count, extension/price chips, and one provider/freshness line. Renewal prices and source evidence remain available under a collapsed details row. The host locale controls number/date formatting, while the provider's actual price currency is preserved; DoName does not infer location from IP or fabricate a currency conversion.

When the host advertises the MCP Apps `serverTools` capability, the View displays a narrow search field that accepts a base name or a full domain. Base names use a curated `.com`, `.fr`, `.ai`, `.io`, `.app` set with `match="any"` and show only provider-confirmed available domains; a full domain routes to the existing read-only `check_domains` tool. Source and exact check time remain inside the expandable details, keeping the compact card free of “checked just now” copy. This does not replace name ideation in the chat composer or send a business brief to GoDaddy. Hosts without that capability keep the fallback MCP conversation and text/structured results; they do not see a nonfunctional input. The updated resource URI is `ui://doname/cards/v3.html`. The V3 layout and search have not yet been checked in a real ChatGPT conversation; run the existing private tunnel test before calling the V3 host experience verified.

No GoDaddy logo is added: its brand guidelines call for permission to use logos, and its current API terms forbid implying endorsement or affiliation without authorization. The plain-text provider attribution remains. The follow-up host-bridge smoke covers base-name search, the five-extension set, exact-domain `tools/call`, available-only rendering, the empty/unverified state, locale/theme updates and teardown. All 38 Python tests pass locally. Real ChatGPT V3 rendering and provider interaction remain for the next private tunnel session.
